...
class CDB_PP_Views {
  public static function init() { add_action( 'template_redirect', array( __CLASS__, 'bump' ) ); }
  protected static function get_ip_hash(){
    $ip = isset($_SERVER['REMOTE_ADDR']) ? $_SERVER['REMOTE_ADDR'] : ''; $ua = isset($_SERVER['HTTP_USER_AGENT']) ? $_SERVER['HTTP_USER_AGENT'] : '';
    return $ip ? substr( md5( $ip . wp_unslash($ua) ), 0, 16 ) : '';
  }
  public static function bump() {
    ...
    $total=intval(get_post_meta($pid,'_views',true)); update_post_meta($pid,'_views',$total+1);
    ...
    $tv=intval(get_post_meta($pid,'_today_views',true)); update_post_meta($pid,'_today_views',$tv+1);
    ...
    $mv=intval(get_post_meta($pid,'_monthly_views',true)); update_post_meta($pid,'_monthly_views',$mv+1);
  }
}