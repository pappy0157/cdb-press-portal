...
class CDB_PP_Loader {
  ...
  // REST endpoint for "新着" infinite load
  ...
  public static function shortcode_portal( $atts = [] ) {
    $pickup = (int) get_option('cdb_pp_pickup_limit', 6 );
    $newper = (int) get_option('cdb_pp_new_limit', 12 );
    ...
    $q2 = new WP_Query([ 'post_type'=>'post','post_status'=>'publish','posts_per_page'=>$newper,'orderby'=>'date','order'=>'DESC' ]);
  ...
  public static function loop_cards( $q ) {
    if ( $q->have_posts() ) {
      while( $q->have_posts() ) { $q->the_post();
        $pid = get_the_ID();
        $company_id = (int) get_post_meta( $pid, get_option('cdb_pp_company_meta_key','_cdb_company_id'), true );
        ...
        if ( $company ) {
          echo '<span class="cdb-pp-company">'.esc_html(get_the_title($company)).'</span>';
        }
        ...
        echo '<button class="cdb-pp-share-btn" data-network="x" title="X">X</button>';
        echo '<button class="cdb-pp-share-btn" data-network="facebook" title="Facebook">Facebook</button>';
        echo '<button class="cdb-pp-share-btn" data-network="line" title="LINE">LINE</button>';
        echo '</div>';
        echo '</article>';
      }
    } else {
