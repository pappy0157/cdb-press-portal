...
class CDB_PP_Social {
  ...
  public static function rest() {
    register_rest_route( 'cdb/v1', '/social-hit', [
      ...
      'callback' => function( WP_REST_Request $req ) {
        ...
        $score = (int) get_post_meta( $post_id, '_social_score', true );
        $score++;
  ...
  // Optional: try to pull Hatena bookmark count (requires external HTTP)
  ...
  public static function decay() {
    // hourly decay: score = floor(score * 0.98)
  ...
  public static function shortcode_social_hot( $atts = [] ) {
    ...
    if ( $q->have_posts() ) {
      ...
      while( $q->have_posts() ){ $q->the_post();
        $s = (int) get_post_meta( get_the_ID(), '_social_score', true );
        echo '<li><a href="'.esc_url(get_permalink()).'">'.esc_html(get_the_title()).'</a><span class="cdb-pp-rank-views">'.number_format_i18n($s).'</span></li>';
      }
      echo '</ol>';
    } else {
      echo '<p class="cdb-pp-empty">'.esc_html__('SNS話題データがありません。','cdb-press-portal').'</p>';
    }
    echo '</div>';
    wp_reset_postdata();
    return ob_get_clean();
  ...
  // REST filter endpoint (used by AJAX filter UI)
  public static function rest_filter( WP_REST_Request $req ) {
    ...
    $args = [
      'post_type'=>'post', 'post_status'=>'publish', 'paged'=>$page, 'posts_per_page'=>12,
    ];
