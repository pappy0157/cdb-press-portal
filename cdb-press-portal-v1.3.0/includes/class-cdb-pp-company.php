...
class CDB_PP_Company {
  ...
  public static function render_box( $post ) {
    ...
    foreach ( $companies as $cid ) {
      $sel = selected( $company_id, $cid, false );
      echo '<option value="'.esc_attr($cid).'" '.$sel.'>'.esc_html(get_the_title($cid)).' (ID:'.$cid.')</option>';
    }
    echo '</select>';
    ?>
    <script>
  ...
  public static function ajax_search() {
    ...
    echo '<option value="">— 未選択 —</option>';
    foreach ( $res as $cid ) {
      echo '<option value="'.esc_attr($cid).'">'.esc_html(get_the_title($cid)).' (ID:'.$cid.')</option>';
    }
    wp_die();
  }
  ...
  public static function save( $post_id ) {
    ...
    if ( $post && $post->post_type === $pt_company ) {
      update_post_meta( $post_id, $meta_key, $company_id );
    }
  }
