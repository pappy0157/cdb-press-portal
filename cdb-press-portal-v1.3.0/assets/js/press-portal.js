(function($){
  ...
  function initHeroSlider($slider){
    ...
    if (count <= 1) { $slider.find('.cdb-pp-slider-nav,.cdb-pp-slider-dots').hide(); return; }
    ...
    function updateDots(){ $dots.children().removeClass('is-active').eq(index).addClass('is-active'); }
    function go(i){
      index = (i + count) % count;
      var x = -index * 100;
      $track.css('transform', 'translateX('+x+'%)'); updateDots();
    }
