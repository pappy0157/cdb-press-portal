 ...
 * Description: プレスリリース特設ページ（/pressportal）を自動生成。ピックアップ・新着・ランキング・SNSで話題・会社連携・タグクラウドAJAXフィルタ・構造化データを提供。
 * Version: 1.1.0
 * Author: CDB Team
 * License: GPL-2.0+
...
// Activation: create portal page & defaults
register_activation_hook( __FILE__, function() {
  ...
  if ( ! $page ) {
    ...
    if ( ! is_wp_error( $page_id ) ) {
      update_option( 'cdb_pp_page_id', $page_id );
    }
  }
...
add_action( 'wp_enqueue_scripts', function() {
  wp_register_style( 'cdb-pp-style', CDB_PP_URL . 'assets/css/press-portal.css', [], CDB_PP_VERSION );
  wp_register_script( 'cdb-pp-script', CDB_PP_URL . 'assets/js/press-portal.js', ['jquery'], CDB_PP_VERSION, true );
  wp_localize_script( 'cdb-pp-script', 'CDBPP', [
    'rest' => [
      'root'  => esc_url_raw( rest_url( 'cdb/v1' ) ),
      'nonce' => wp_create_nonce( 'wp_rest' ),
    ],
    'labels' => [
      'more'   => __('もっと見る', 'cdb-press-portal'),
      'loading'=> __('読み込み中…', 'cdb-press-portal')
    ]
  ]);
});
