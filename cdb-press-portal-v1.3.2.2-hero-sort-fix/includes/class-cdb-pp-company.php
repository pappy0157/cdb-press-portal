...
class CDB_PP_Company {
  public static function init() {
    ...
    add_action( 'wp_ajax_cdb_pp_company_search', array( __CLASS__, 'ajax_search' ) );
  }
  public static function add_box( $post_type ) {
    if ( 'post' !== $post_type ) return;
    add_meta_box( 'cdb_pp_company_box', '会社を選択', array( __CLASS__, 'render_box' ), 'post', 'side', 'default' );
  }
  public static function render_box( $post ) {
    $meta_key   = get_option('cdb_pp_company_meta_key','_cdb_company_id');
    $pt_company = get_option('cdb_pp_company_post_type','company');
    $company_id = intval( get_post_meta( $post->ID, $meta_key, true ) );
    ...
    $companies = get_posts(array( 'post_type'=>$pt_company, 'post_status'=>'publish', 'posts_per_page'=>50, 'orderby'=>'date', 'order'=>'DESC', 'fields'=>'ids' ));
    ...
    ?>
    <script>
      function cdbPPcompanySearch(q){
        if(!q || q.length < 2){ return; }
        jQuery.get(ajaxurl, { action: 'cdb_pp_company_search', q: q }, function(html){
          if(html){ jQuery('#cdb_pp_company_id').html(html); }
        });
      }
    </script>
    <?php
  ...
  public static function ajax_search() {
    ...
    $res = get_posts(array( 'post_type'=>$pt, 'post_status'=>'publish', 's'=>$q, 'posts_per_page'=>50, 'fields'=>'ids' ));
    echo '<option value="">— 未選択 —</option>';
