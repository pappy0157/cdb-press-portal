 ...
 * Description: プレスリリース特設ページ（/pressportal）を自動生成。ピックアップ・新着・ランキング・SNSで話題・会社連携・タグクラウドAJAXフィルタ・構造化データ。ヒーローはスライダー、フィルタはヒーロー直下に表示。
 * Version: 1.3.2.2
 * Author: CDB Team
 * License: GPL-2.0+
...
require_once CDB_PP_DIR . 'includes/class-cdb-pp-views.php';
...
add_action( 'wp_enqueue_scripts', function() {
  wp_enqueue_style( 'cdb-pp-style', CDB_PP_URL . 'assets/css/press-portal.css', array(), CDB_PP_VERSION );
  wp_enqueue_script( 'cdb-pp-script', CDB_PP_URL . 'assets/js/press-portal.js', array('jquery'), CDB_PP_VERSION, true );
  wp_localize_script( 'cdb-pp-script', 'CDBPP', array(
    'rest' => array( 'root'  => esc_url_raw( rest_url( 'cdb/v1' ) ), 'nonce' => wp_create_nonce( 'wp_rest' ) ),
    'labels' => array( 'more' => __('もっと見る','cdb-press-portal'), 'loading' => __('読み込み中…','cdb-press-portal'),
