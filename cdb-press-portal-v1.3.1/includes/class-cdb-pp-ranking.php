...
class CDB_PP_Ranking {
  ...
  public static function shortcode_today_rank( $atts = [] ) {
    ...
    $q = new WP_Query([ 'post_type'=>'post','post_status'=>'publish','posts_per_page'=>(int)$atts['limit'],'orderby'=>'meta_value_num','order'=>'DESC','meta_key'=>'_today_views' ]);
    ob_start();
    echo '<ol class="cdb-pp-list cdb-pp-rank">';
    if ( $q->have_posts() ) { while ( $q->have_posts() ) { $q->the_post();
      self::list_item_with_thumb( function( $post_id ){ $views=(int)get_post_meta( $post_id, '_today_views', true ); return '<span class="cdb-pp-rank-views">'.number_format_i18n($views).'</span>'; });
  ...
  public static function shortcode_month_rank( $atts = [] ) {
    $atts = shortcode_atts([ 'limit' => 10 ], $atts );
    $q = new WP_Query([ 'post_type'=>'post','post_status'=>'publish','posts_per_page'=>(int)$atts['limit'],'orderby'=>'meta_value_num','order'=>'DESC','meta_key'=>'_monthly_views' ]);
    ob_start();
    echo '<ol class="cdb-pp-list cdb-pp-rank">';
    if ( $q->have_posts() ) { while ( $q->have_posts() ) { $q->the_post();
      self::list_item_with_thumb( function( $post_id ){ $views=(int)get_post_meta( $post_id, '_monthly_views', true ); return '<span class="cdb-pp-rank-views">'.number_format_i18n($views).'</span>'; });
    } } else { echo '<li class="cdb-pp-list-item">'.esc_html__('ランクデータがありません。','cdb-press-portal').'</li>'; }
    echo '</ol>'; wp_reset_postdata(); return ob_get_clean();
  }
}