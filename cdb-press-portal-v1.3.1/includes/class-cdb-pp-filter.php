...
class CDB_PP_Filter {
  public static function init() { add_shortcode( 'cdb_press_filter', [ __CLASS__, 'shortcode' ] ); }
  public static function shortcode( $atts = [] ) {
    ...
    echo '</section>';
    ?> <script>(function($){$(document).on('click','.cdb-pp-tag',function(){var slug=$(this).data('tag')||'';$('.cdb-pp-filter-tag').remove();$('<input>',{type:'hidden',class:'cdb-pp-filter-tag',value:slug}).appendTo('.cdb-pp-section .cdb-pp-filter');$('.cdb-pp-filter-do').trigger('click');});})(jQuery);</script> <?php
    return ob_get_clean();
  }
}